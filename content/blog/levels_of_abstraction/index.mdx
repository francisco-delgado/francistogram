...
If you ever have a moment when you think to yourself, "Hmm, something just doesn't make sense". Try again - you haven't abstracted the problem enough.
...
You apply Occam's Razor knowing there exists is a simpler solution. You add a layer of abstraction and you start to consider it must be a single realtor that owns both listings. Something still doesn't make sense. It's as if they spent all the time to write up the message
...
Again you question your assumption that it's a single realtor.
...
You apply Occam's Razor knowing there exists is a simpler solution. The realtor owns at least a two homes and so you can assume he's wealthy. Knowing that wealth compounds continuously odds are good that: given he owns two homes he owns more than two.

It's conceivable he's so wealthy it's not worth his time to add an automated response to all of them and outsourced it to an intern.

He doesn't trust the intern to write the message because a bad message would cause him to lose business and quickly creates `automated_response` himself and passes it to an intern.
